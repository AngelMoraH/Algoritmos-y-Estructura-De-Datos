...
public:
  ...
  bool solve_query(T X, int query_type) { return this->find(X); }
  ...
  ForwardList<int> &range_search(ForwardList<int> &indexes, NodeBT<T> *root,
                                 int k1, int k2) {
    if (NULL == root)
      return indexes;
  ...
  void remove(T value) {
    ...
    while (flag) {
      if (value > trav->data) {
        ...
        // return true; found
        if (trav->right == nullptr && trav->left == nullptr) { // it is a leaf
          if (prev->left == trav) {
            prev->left = nullptr;
  ...
  void buildFromOrderedArray(
      int n) { // dado un array ordenado construir un arbol binario balanceado
...
private:
  ...
  bool isBalanced(NodeBT<T> *node) {
    ...
    if (abs(th - rh) <= 1 && isBalanced(node->left) &&
